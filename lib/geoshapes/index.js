...
const pgPromise = require( 'pg-promise' )( { promiseLib: BBPromise } );
...
function handler( core, config, type, req, res, next ) {
	...
	return BBPromise.try( () => shape.execute( type, req.query, lowerHeaders[ 'x-client-ip' ], core.metrics )
	).then( ( geodata ) => {
...
function loadServiceConfig( core ) {
	return BBPromise.try( () => {
		...
		// TODO: we might want to setup and use the apiUtil here with a proper host
		const mwapi = core.getConfiguration().mwapi_req;
		config.mwapi = ( mwapi && mwapi.uri && !mwapi.uri.includes( '{{domain}}' ) ) ?
			mwapi.uri : 'https://en.wikipedia.org/w/api.php';
...
function loadDBHandler( config ) {
	return BBPromise.try( () => {
		...
		config.db = pgPromise( {
			...
			// additional settings will be forwarded to node-postgres
