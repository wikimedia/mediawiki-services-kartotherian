...
function handler( core, config, type, req, res, next ) {
	...
	return BBPromise.try( () => {
		...
		return geoshape.execute( lowerHeaders[ 'x-client-ip' ], core.metrics );
	} ).then( ( geodata ) => {
...
function loadServiceConfig( core ) {
	return BBPromise.try( () => {
		...
		const userAgentContact = core.getConfiguration().userAgentContact;
		config.userAgent = `${packageInfo.name}/${packageInfo.version} ${userAgentContact}`;
