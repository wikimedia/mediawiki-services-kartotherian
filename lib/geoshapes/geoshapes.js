...
class GeoShapes {
	...
	async execute( xClientIp, metrics ) {
		...
		const result = await this._wrapResult( geoRows, cleanProperties, this.useGeoJson );
	...
	async _runWikidataQuery( xClientIp ) {
		...
		if ( !this.sparqlQuery && this.ids && this.type === 'geopoint' ) {
			this.sparqlQuery = this._createPointsSparqlQuery( this.ids );
		}
