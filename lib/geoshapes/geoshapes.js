...
class GeoShapes {
	...
	async execute( type, reqParams, xClientIp, metrics ) {
		...
		if ( allIds.length !== 0 && type !== 'geopoint' && ( !geoRows || geoRows.length === 0 ) &&
			...
			core.log( 'warn', 'No results from ' + type + ' local database request' );
		}
	...
	_processResultRow( wd, customIdColumn ) {
		const idColumn = customIdColumn || defaultIdColumn;
		if ( !( idColumn in wd ) ) {
	...
	_wrapResult( type, rows, allProperties, useGeoJson ) {
		...
		if ( type === 'geopoint' && allProperties ) {
			features = allProperties.map( ( props ) => {
				const id = props.id;
				delete props.id;
				// Note: At this point the ….geo field doesn't contain a Wikidata value any more but
				// an array we can use in GeoJSON as-is. Conversion happened in _expandProperties().
				const coordinates = props[ geoColumn ];
				delete props[ geoColumn ];
