...
class GeoShapes {
	...
	constructor( type, reqParams, config ) {
		...
		let ids = reqParams.ids ? reqParams.ids.split( ',' ) : [];
		ids = ids.filter( ( id ) => {
			if ( id && !/^Q[1-9][0-9]{0,15}$/.test( id ) ) {
				throw new Err( 'Invalid Wikidata id "%s"', id );
			}
			return id;
		} );
		if ( ids.length > this.config.maxidcount ) {
	...
	async _expandProperties( rawProperties ) {
		...
		const props = rawProperties.map( ( row ) => {
			const { id, ...prop } = row;
			// TODO: Object.fromEntries with node 12
			const transformedProps = _.object(
				Object.entries( prop )
					.filter( ( [ , raw ] ) => raw )
					.map( ( [ key, raw ] ) => {
						...
						const value = parseWikidataValue( raw );
