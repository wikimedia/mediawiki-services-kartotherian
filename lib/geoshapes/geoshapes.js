...
class GeoShapes {
	...
	constructor( type, reqParams, config ) {
		...
		if ( reqParams.ids ) {
			this.ids = new Set( reqParams.ids.split( ',' ).filter( ( id ) => id !== '' ) );
			if ( this.ids.size > this.config.maxidcount ) { throw new Err( 'No more than %d IDs is allowed', this.config.maxidcount ); }
