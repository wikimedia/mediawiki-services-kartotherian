...
function getLayerFilter( opts ) {
	...
	return ( xmlChild ) => _.contains( layers, xmlChild.getAttribute( 'name' ).value() ) === include;
}
...
XmlLoader.prototype.update = function update( xmlData, xmlFile ) {
	...
	const doc = libxmljs.parseXml( xmlData, { preserveWhitespace: true } );
	...
	return doc.toString( { format: false } );
};
