...
core.registerTileliveModule = function registerTileliveModule( moduleName ) {
	...
	if ( !core._registeredModules[ moduleName ] ) {
		...
		if ( moduleName === '@mapbox/tilelive-vector' ) {
			// Override default setting, so we don't load the map file in strict mode. See T195513
			module.strict = false;
		}
...
core.reportRequestError = function reportRequestError( err, res ) {
	...
	core.reportError( ( err ) => {
		...
		const mx = ( core.areMetricsValid( err.metrics ) && err.metrics ) || 'err.unknown';
		core.metrics.makeMetric( {
			type: 'Counter',
			name: mx,
			prometheus: {
				name: 'kartotherian_' + mx.replace( '.', '_' ) + '_total',
				help: 'Error count'
			}
		} ).increment();
	}, err );
