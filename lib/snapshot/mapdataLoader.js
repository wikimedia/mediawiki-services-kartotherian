...
function flattenArraysAndFeatureCollections( features, geoJSON ) {
	if ( Array.isArray( geoJSON ) ) {
		...
		if ( !geoJSON.type ) { throw new Error( 'Bad GeoJSON - object has no "type" property' ); }
		switch ( geoJSON.type ) {
		...
		throw new Error( `Bad GeoJSON - unknown JSON type "${typeof ( geoJSON )}"` );
	}
}
...
function downloadMapdata( req, protocol, domain, title, revid,
	...
	return dm.loadGroups( groupIds ).then( ( dataGroups ) => {
		...
		// sorting geopoints north to south
		mapdata.sort( ( a, b ) => {
...
module.exports = {
	downloadMapdata,
	private: {
		flattenArraysAndFeatureCollections
	}
};
