		...
		return mapdataLoader(
			...
			const renderOverlayMap = Promise.try( () => new Promise( ( accept, reject ) => {
				...
				if ( geoJSON.type === 'FeatureCollection' && geoJSON.features.length === 0 ) {
					const err = new Err( 'Empty geojson feature collection' );
					err.name = 'EmptyGeoJSONFeatureCollection';
					throw err;
				}
			...
			return Promise.join(
				renderBaseMap,
				renderOverlayMap.catch( ( err ) => {
					if ( err.name === 'EmptyGeoJSONFeatureCollection' ) {
						return null;
					}
					throw err;
				} ),
